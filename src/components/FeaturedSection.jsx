...
/**
 * Display-only featured row (Phase 0).
 * Admin feature/unfeature is managed exclusively via Admin panel —
 * no client star toggle that wrote a single `featured` column.
 */
export default function FeaturedSection({ featured, navigate }) {
    ...
    <div style={S.section}>
      {!showAll ? (
        ...
        <>
          <div style={S.header}>
            <div style={S.headerLeft}>
              <button style={S.backBtn} onClick={() => setShowAll(false)}>
              ...
              <span style={S.title}>All featured</span>
            </div>
            <span style={S.countPill}>{featured.length} listings</span>
          </div>
...
function FeaturedCard({ item, index: i, navigate, size }) {
