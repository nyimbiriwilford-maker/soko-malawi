...
<div style={{ height: 24 }} />
  ...
  {/* Copy link row */}
  <button onClick={(e) => { e.stopPropagation(); copyLink() }} style={S.shareRow}>
    <div style={{ ...S.shareRowIcon, background: copied ? '#e6f4ec' : '#f3f4f6' }}>
      {copied
        ? <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1a7a4a" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
        : <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#374151" strokeWidth="2" strokeLinecap="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
      }
    </div>
    <span style={{ color: copied ? '#1a7a4a' : '#0f1410' }}>{copied ? 'Copied!' : 'Copy link'}</span>
  </button>
  {/* WhatsApp row */}
  <a href={`https://wa.me/?text=${encodeURIComponent(listing.title + ' — ' + window.location.href)}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={S.shareRow}>
    <div style={{ ...S.shareRowIcon, background: '#dcfce7' }}>
    ...
    <span style={{ color: '#0f1410' }}>Share on WhatsApp</span>
  </a>
  {/* Facebook row */}
  <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={S.shareRow}>
    <div style={{ ...S.shareRowIcon, background: '#dbeafe' }}>
      <svg width="20" height="20" viewBox="0 0 24 24" fill="#1d4ed8"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>
    </div>
    <span style={{ color: '#0f1410' }}>Share on Facebook</span>
  </a>
  {/* X/Twitter row */}
  <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(listing.title)}&url=${encodeURIComponent(window.location.href)}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={S.shareRow}>
    <div style={{ ...S.shareRowIcon, background: '#f0f9ff' }}>
      <svg width="20" height="20" viewBox="0 0 24 24" fill="#0ea5e9"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.748l7.73-8.835L1.254 2.25H8.08l4.259 5.63 5.905-5.63zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
    </div>
    <span style={{ color: '#0f1410' }}>Post on X</span>
  </a>
</div>
          </div>
        </div>
...
shareRow: { display: 'flex', alignItems: 'center', gap: 14, padding: '12px 4px', background: 'none', border: 'none', cursor: 'pointer', width: '100%', textDecoration: 'none', fontSize: 15, fontWeight: 600 },
shareRowIcon: { width: 44, height: 44, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
}